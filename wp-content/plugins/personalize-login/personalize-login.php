...
class Personalize_Login_Plugin {
	...
	public function redirect_to_custom_login() {
		if ( $_SERVER['REQUEST_METHOD'] == 'GET' ) {
			...
			$login_url = home_url( get_field('login_url', 'option') );
	...
	public function redirect_after_login( $redirect_to, $requested_redirect_to, $user ) {
		...
		if ( user_can( $user, 'manage_options' ) ) {
			...
			$redirect_url = home_url( get_field('member_account', 'option') );
		}
	...
	public function redirect_after_logout() {
		$redirect_url = home_url( get_field('logged_out', 'option') );
	...
	public function redirect_to_custom_lostpassword() {
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			wp_redirect( home_url( get_field('member_password_lost_url', 'option') ) );
			exit;
		}
	}
		...
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( ! $user || is_wp_error( $user ) ) {
				if ( $user && $user->get_error_code() === 'expired_key' ) {
					wp_redirect( home_url( get_field('login_expiredkey', 'option') ) );
				} else {
					wp_redirect( home_url( get_field('login_invalidkey', 'option') ) );
				}
				exit;
			}

			$redirect_url = home_url( get_field('member_password_reset', 'option') );
	...
	public function do_password_lost() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( is_wp_error( $errors ) ) {
				// Errors found
				$redirect_url = home_url( get_field('member_password_lost_url', 'option') );
				$redirect_url = add_query_arg( 'errors', join( ',', $errors->get_error_codes() ), $redirect_url );
			} else {
				// Email sent
				$redirect_url = home_url( get_field('login_url', 'option') );
	...
	public function do_password_reset() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( isset( $_POST['pass1'] ) ) {
				...
				wp_redirect( home_url( get_field('login_pass_changed', 'option') ) );
			} else {
	...
	public function replace_retrieve_password_message( $message, $key, $user_login, $user_data ) {
		$resetlink = get_field('member_password_reset', 'option');
		// Create new message
		...
		$msg .= site_url( "$resetlink?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . "\r\n\r\n";
	...
	private function redirect_logged_in_user( $redirect_to = null ) {
		...
		if ( user_can( $user, 'manage_options' ) ) {
			...
			wp_redirect( home_url( get_field('member_account', 'option') ) );
		}
	}
	...
	private function get_error_message( $error_code ) {
		switch ( $error_code ) {
			...
			case 'incorrect_password':
				...
				return sprintf( $err, get_field('member_password_lost_url', 'option') );
